...
```{r}
#| label: extract_historical_population_estimates
...
historical_population_estimates_df <- bind_rows(Map(
    function(parameter, estimate) {
        tibble(
            parameter = parameter,
            component = seq_along(estimate),
            group = if (isTRUE(grepl("bsld", parameter))) {
                "historical"
            } else {
                historical_arm_names
            },
            estimate = estimate
        )
    },
    names(historical_population_estimates),
    historical_population_estimates
))
...
```{r}
#| label: create_current_joint_data
...
We then specify a constant (also called Dirac or point-mass) prior for each of the six historical population estimates. `prior_const()` supplies the single historical-study value for each baseline parameter; because the current data also contain one study, `jmpost` replicates that scalar to the parameter's study dimension. `prior_const_vector()` supplies the arm-specific shrinkage and growth parameters in the same arm order as shown above. The subject-level random effects and the remaining joint-model parameters are still estimated from the current data, conditional on these fixed population values.

```{r}
#| label: specify_sequential_current_model
...
```{r}
#| label: fit_sequential_current_model
...
As before, we inspect the posterior summary of the fitted model. An `NA` value for `rhat` is expected for a fixed parameter: all its draws are identical, so the between- and within-chain variances needed for this diagnostic are both zero.

```{r}
#| label: summarize_sequential_current_model
...
Finally, we compare uncertainty in the population parameters used for the current-study analysis under the two approaches. For the hierarchical fit we select the current-study component of the baseline parameters. In this model specification, the shrinkage and growth parameters are arm-specific and shared between studies, so they already have the same dimension in both fits.

```{r}
#| label: compare_sequential_bhjm_uncertainty

current_study_index <- unname(
    as_stan_list(pooled_subj_data)$study_to_index["current"]
)
baseline_vars <- c("lm_sf_mu_bsld", "lm_sf_omega_bsld")
baseline_current_vars <- paste0(
    baseline_vars,
    "[",
    current_study_index,
    "]"
...
bhjm_current_population_summary <- mcmc_pooled_joint_results$summary(c(
    baseline_current_vars,
    setdiff(population_vars, baseline_vars)
))
sequential_population_summary <-
    mcmc_current_sequential_results$summary(population_vars)

# Both baseline parameters have index 1 in the current-only fit. Give the
# corresponding BHJM parameters the same labels before joining the summaries.
bhjm_current_population_summary$parameter <-
    bhjm_current_population_summary$variable
baseline_rows <- match(
    bhjm_current_population_summary$variable,
    baseline_current_vars
)
replace_rows <- !is.na(baseline_rows)
bhjm_current_population_summary$parameter[replace_rows] <- paste0(
    baseline_vars[baseline_rows[replace_rows]],
    "[1]"
)

population_uncertainty_comparison <- sequential_population_summary |>
    transmute(
        parameter = variable,
    ...
    left_join(
        bhjm_current_population_summary |>
            transmute(
                parameter,
                bhjm_mean = mean,
                bhjm_sd = sd
    ...
    select(parameter, bhjm_mean, sequential_mean, bhjm_sd, sequential_sd)

population_uncertainty_comparison
```

The sequential estimates have posterior standard deviation zero by construction, and hence appear much more precise than the corresponding BHJM estimates. This is not additional information supplied by the current study: it is a direct consequence of treating estimates from the historical study as known with 100% certainty. The BHJM retains uncertainty about the population parameters used for the current-study analysis and therefore gives the more realistic assessment of precision.
