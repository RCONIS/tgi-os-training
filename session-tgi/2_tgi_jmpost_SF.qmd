...
mcmc_hist(vars_draws)

# But now we want to overlay the posterior and prior:
...
post_sum <- post_samples |>
  ...
  select(b0, ks, kg, omega_bsld, omega_ks, omega_kg, sigma) |>
  summarize_draws() |>
  gt() |>
...
With `jmpost`, it is easy to compute the LOO criterion:
...
Underneath this is using the [$loo()](https://mc-stan.org/cmdstanr/reference/fit-method-loo.html) method from `cmdstanr`. A helpful glossary explaining the LOO statistics can be found [here](https://mc-stan.org/cmdstanr/reference/loo-glossary.html).
