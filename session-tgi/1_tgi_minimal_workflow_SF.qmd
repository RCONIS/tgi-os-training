...
$$
\mu(t_{ij}) = \mu_{0i} \{\exp(-s_i \cdot t_{ij}) + \exp(g_i \cdot t_{ij}) - 1\}
$$
...
\mu_{0i} \{\exp(-s_i \cdot t_{ij}) + \exp(g_i \cdot t_{ij}) - 1\} & \text{if } t_{ij} \geq 0 
\end{cases}
$$
...
Next, we define the distributions of the random effects $\mu_{0i}$, $s_i$, $g_i$, for $i = 1, \dotsc, n$:
...
# Fit the model
save_file <- here("session-tgi/fit4.RData")
...
In total this took 37 minutes on my laptop. 
...
```{r}
mcmc_trace(post_df, pars = c("mu_0", "mu_s", "mu_g", "sigma"))
mcmc_pairs(post_df, pars = c("mu_0", "mu_s", "mu_g", "sigma"))

post_sum <- post_df |>
  select(mu_0, mu_s, mu_g, omega_0, omega_s, omega_g, sigma) |>
  summarize_draws() |>
  gt() |>
...
Let's code the function that gives the SF mean:

```{r}
sf_mean <- function(mu0, s, g, day) {
  ifelse(day < 0, mu0 * exp(g * day), mu0 * (exp(-s * day) + exp(g * day) - 1))
}
```
...
sum_ind_1 <- post_ind_1 |> 
  cross_join(data.frame(day = (-7):100)) |> 
  mutate(mu = sf_mean(mu0, s, g, day)) |> 
  group_by(day) |> 
