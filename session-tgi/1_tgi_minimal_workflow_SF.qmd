...
For comparison, let's fit the additive model to see if that gives a better fit.

```{r}
#| label: fit_brms
formula <- bf(sld ~ step(year > 0) * 
        (b0 * (exp(-ks * year) + exp(kg * year) -  1)) +
      step(year <= 0) * 
        (b0 * exp(kg * year)), nl = TRUE) +
  # Define nonlinear parameter transformations
  nlf(b0 ~ exp(lb0)) +
  nlf(ks ~ exp(lks)) +
  nlf(kg ~ exp(lkg)) +
...
# Fit the model
save_file <- here("session-tgi/fit7.RData")
if (file.exists(save_file)) {
  load(save_file)
} else {
  fit <- brm(
    ...
    family = gaussian(),
    init = rep(list(inits), CHAINS),
    chains = CHAINS, iter = 2000, warmup = 1000, seed = BAYES.SEED, 
    refresh = REFRESH, thin = 2
  )
  save(fit, file = save_file)
}
...
Let's look again at the model fit:


```{r}
#| label: obs_vs_add_fit_plots
...
df_sim |>
  ...
  geom_line(aes(y = sld_pred), color = "deepskyblue") +
  geom_point(data = df_subset, color = "tomato") +
  geom_point(data = df_subset, aes(y = pm), color = "green") +
  coord_cartesian(ylim = range(df_subset$sld)) +
  scale_fill_brewer(palette = "Greys") +
  labs(title = "SF model fit")
```
