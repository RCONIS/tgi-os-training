...
save_file <- here("session-tgi/jm3.RData")
if (file.exists(save_file)) {
  load(save_file)
} else {
...
mcmc_res_cmdstan_by_arm$summary(vars)
```
...
post_samples_by_arm <- as_draws_df(mcmc_res_cmdstan_by_arm$draws(vars)) |> 
  rename(
    mu_bsld = "lm_sf_mu_bsld[1]",
  ...
  mutate(
    b0 = exp(mu_bsld + omega_bsld^2 / 2), 
    ks1 = exp(mu_ks1 + omega_ks1^2 / 2), 
    ks2 = exp(mu_ks2 + omega_ks2^2 / 2),
    kg1 = exp(mu_kg1 + omega_kg1^2 / 2),
    kg2 = exp(mu_kg2 + omega_kg2^2 / 2),
    cv_0 = sqrt(exp(omega_bsld^2) - 1),
...
```{r}
prob_ks1_greater_ks2 <- mean(post_samples_by_arm$ks1 > post_samples_by_arm$ks2)
...
loo_res_by_arm <- mcmc_res_cmdstan_by_arm$loo(r_eff = FALSE)
loo_res_by_arm
```
