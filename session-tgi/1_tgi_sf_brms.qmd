...
Therefore we can add this as follows as a fixed effect for the two parameters:

```{r}
#| label: formula_covariates

formula_by_arm <- bf(sld ~ eta, nl = TRUE) +
  # Define the mean for the likelihood
  nlf(
    eta ~ 
      int_step(year > 0) * 
...
It is instructive to see that the Stan code that is generated in the background is actually identical to the previous model. This is because `brms` uses a design matrix to model the fixed effects, and the `arm` variable is just added to this design matrix, which before only contained the intercept column with `1`s. 

However, now the coefficient vector is no longer of length 1 but of length 2, with the first element corresponding to the intercept and the second element to the effect of the `arm` variable. For the latter, we want to assume a standard normal prior on the log scale. 
