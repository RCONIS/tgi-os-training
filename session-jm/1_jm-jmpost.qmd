...
surv_data <- DataSurvival(
    data = os_data,
    formula = Surv(os_time, os_event) ~ arm + ecog + age + race + sex
)
```

Note that we don't include the growth rate estimate `kg_est` here, because here we are fitting a joint model - the growth rate will be included later in the `JointModel` specification instead.
...
We specify the Stein-Fojo model for the TGI data, the Weibull model for the OS data, as well as the link, i.e. the growth rate from the TGI model which shall influence the hazard in the OS model:
...
draws_joint_results <- do.call(
    rename_variables,
    c(list(draws_joint_results), setNames(old_coef_names, os_cov_names))
)
...
- the `arm` effect is less pronounced here (Atezo is still lowering the hazard but slightly less here)
- the `kg_est` effect (see `link_growth` here) is stronger here
...
We can see that the fit looks adequate, with the modelled survival functions closely following the Kaplan-Meier curves in each treatment group.
...
Surprisingly, also here the hazard ratio is indeed constant over time, which was the same in the 2-step TGI-OS model.
This is because the link between the longitudinal and the survival model is here the growth rate of the Stein-Fojo model, which is constant over time. For other link functions that are time-varying, e.g. the derivative of the longitudinal model, the hazard ratio could change over time.

So here the estimated hazard ratio is `r round(os_hr_est$mean[1], 2)` with a 90% credible interval of `r round(os_hr_est$lower[1], 2)` to `r round(os_hr_est$upper[1], 2)`. We can see that this estimate is slightly closer to 1, and that the 90% CI now actually includes 1, in contrast to the 2-step TGI-OS model.
...
We can again use the [Brier score](https://en.wikipedia.org/wiki/Brier_score) to compare the two different survival models as part of the joint models. The Brier score is a measure of the mean squared difference between the predicted survival probability and the actual survival status. The lower the Brier score, the better the model.
...
Now let's compare this with the second model where we have the derivative link. We have a suspicion that the second model will perform worse, because of the hazard ratio results, but let's have a look.
...
We can plot the Brier scores to compare them visually:
