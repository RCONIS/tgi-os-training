...
os_fixed_surv@data@survival@data$os_event <- as.numeric(os_fixed_surv@data@survival@data$os_event)
...
save_file <- here("session-os/os2.RData")
if (file.exists(save_file)) {
  load(save_file)
} else {
  ...
  save(os_results2, file = save_file)
}

save_file <- here("session-os/os2-mcmc.rds")
if (file.exists(save_file)) {
  mcmc_os_results2 <- readRDS(save_file)
} else {
  mcmc_os_results2 <- cmdstanr::as.CmdStanMCMC(os_results2)
  mcmc_os_results2$save_object(file = save_file)
}
```
...
# Current workaround if we have a logical event indicator:
os_fixed_surv2@data@survival@data$os_event <- as.numeric(os_fixed_surv2@data@survival@data$os_event)
