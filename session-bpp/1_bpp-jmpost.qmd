...
os_data_samples <- lapply(1:ITER, function(i) {
    ...
    os_data_sample <- os_data[, c("id", "arm", "os_time", "os_event")]
...
log_rank_at_ia <- log_rank_stats_fun(os_data)
log_rank_at_ia
...
So we see that this PTS with `r round(pts_log_rank * 100, 1)`% is much lower than the previous PTS estimates. Given that the log-rank test statistic at interim is only `r round(log_rank_at_ia, 2)`, and the critical value is `r round(critical_value, 2)`, it is not surprising that the PTS is low.
...
So this PTS estimate is in line with the above log-rank PTS estimate.
...
survfit_ia <- survival::survfit(Surv(os_time, os_event) ~ arm, data = os_data)
